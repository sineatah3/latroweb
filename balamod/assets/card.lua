LOVELY_INTEGRITY = 'aa91aadb9459747ac924fd754968ff837bbaed6d47ee18019d463bb3436af460'
    ...
    if self.facing == 'front' and (not self.states.drag.is or G.CONTROLLER.HID.touch) and not self.no_ui and not G.debug_tooltip_toggle then 
        ...
        self.config.h_popup = G.UIDEF.card_h_popup(self)
                if self.ability.retriggers and self.ability.retriggers >= 1 then
                    local t = G.UIDEF.card_h_popup(self)
                    local badges = t.nodes[1].nodes[1].nodes[1].nodes[3].nodes
                    badges[#badges + 1] = create_badge(localize('s_energy'), HEX("6495ed"), nil, nil)
                    self.config.h_popup = t
                end
